...
public class ArticleService
{
	...
	public ArticlePage getArticlePage(HttpServletRequest request)
	{
		...
		articleCount = categoryId == 0 ? articleDao.selectArticleCountAll(search)
		        : articleDao.selectArticleCountCategory(search, categoryId);
		...
		articleList = categoryId == 0
		        ? articleDao.selectArticleListAll(search, (currentPage - 1) * ARTICLE_PER_PAGE, ARTICLE_PER_PAGE)
		        : articleDao.selectArticleListCategory(search, categoryId, (currentPage - 1) * ARTICLE_PER_PAGE,
		                ARTICLE_PER_PAGE);
	...
	public ArticlePage getArticlePageHot(HttpServletRequest request)
	{
		ArticleDao articleDao = ArticleDao.getInstance();
		articleDao.startConnection();
		...
		articleCount = articleDao.selectArticleCountHot(search);
		currentPage = selectPage > 0 ? selectPage : 1;
		totalPage = articleCount % ARTICLE_PER_PAGE != 0 ? articleCount / ARTICLE_PER_PAGE + 1
		        : articleCount / ARTICLE_PER_PAGE;
		startPage = currentPage > VIEW_PAGES / 2 ? currentPage - VIEW_PAGES / 2 + 1 : 1;
		...
		startPage = totalPage == endPage && endPage > ARTICLE_PER_PAGE && startPage > ARTICLE_PER_PAGE
		        ? endPage - ARTICLE_PER_PAGE + 1 : startPage;
		articleList = articleDao.selectArticleListHot(search, (currentPage - 1) * ARTICLE_PER_PAGE, ARTICLE_PER_PAGE);
		pageArticleCount = articleList.size();
		System.out.println("pageArticleCount = " + pageArticleCount);
		System.out.println(
		        String.format("startPage = %d, endPage = %d, currentPage = %d, selectPage = %d, totalPage = %d",
	...
	public ArticlePage getArticlePagePremium(HttpServletRequest request)
	{
		ArticleDao articleDao = ArticleDao.getInstance();
		articleDao.startConnection();
		...
		articleCount = articleDao.selectArticleCountPremium(search);
		currentPage = selectPage > 0 ? selectPage : 1;
		totalPage = articleCount % ARTICLE_PER_PAGE != 0 ? articleCount / ARTICLE_PER_PAGE + 1
		        : articleCount / ARTICLE_PER_PAGE;
		startPage = currentPage > VIEW_PAGES / 2 ? currentPage - VIEW_PAGES / 2 + 1 : 1;
		...
		startPage = totalPage == endPage && endPage > ARTICLE_PER_PAGE && startPage > ARTICLE_PER_PAGE
		        ? endPage - ARTICLE_PER_PAGE + 1 : startPage;
		articleList = articleDao.selectArticleListPremium(search, (currentPage - 1) * ARTICLE_PER_PAGE,
		        ARTICLE_PER_PAGE);
		pageArticleCount = articleList.size();
		System.out.println("pageArticleCount = " + pageArticleCount);
		System.out.println(
